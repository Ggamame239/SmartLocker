...
app.post('/api/rfid/scan', async (req, res) => {
  // ESP32 ต้อง POST มาที่ endpoint นี้ พร้อม header X-Device-Key และ body JSON { cardUid: '<UID>' }
  // นี่คือจุดเชื่อมต่อหลักสำหรับตัวอ่าน RFID บน ESP32 กับ backend.
  ...
  try {
    const occupied = await admin.database().ref(`rfidCards/${cardUid}`).get();
    if (occupied.exists()) return res.status(409).json({ message: 'บัตรนี้ถูกยืนยันแล้ว' });
