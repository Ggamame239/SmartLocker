...
// Owner session store (in-memory). For production, replace with persistent store.
const crypto = require('crypto');
const ownerSessions = new Map(); // token -> { username, expiresAt }
const OWNER_SESSION_TTL = 1000 * 60 * 60 * 4; // 4 hours

// Middleware to protect owner APIs
async function ownerAuth(req, res, next) {
  try {
    const token = req.cookies?.ownerSession || req.headers['x-owner-session'] || req.query.session;
    if (!token) return res.status(401).json({ message: 'Owner not authenticated' });
    const s = ownerSessions.get(token);
    if (!s || s.expiresAt < Date.now()) {
      if (s) ownerSessions.delete(token);
      return res.status(401).json({ message: 'Owner session expired' });
    }
    req.owner = { username: s.username };
    // refresh expiry
    s.expiresAt = Date.now() + OWNER_SESSION_TTL;
    ownerSessions.set(token, s);
    next();
  } catch (err) {
    console.error('ownerAuth error', err);
    res.status(500).json({ message: 'Owner auth error' });
  }
}

// expose owner static page at /owner (SPA)
...
app.post('/api/rfid/scan', async (req, res) => {
  ...
  try {
    ...
    if (snap.exists()) dbDeviceKey = String(snap.val()).trim();
  } catch (e) {
